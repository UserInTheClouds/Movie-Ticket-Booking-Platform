    ...
    const handlePayment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                navigate('/booking/success', { state: { booking: bookingData, showtime, selectedSeats } });
            } else {
                ...
                <form onSubmit={handlePayment} className="bg-white rounded-xl shadow-sm p-4">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input required type="text" placeholder="Name on card" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#584cf4]" />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1">Card number</label>
                            <input required type="text" placeholder="1234 5678 9012 3456" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#584cf4]" />
                        </div>
                        <div className="flex space-x-4">
                            <div className="flex-1">
                                <label className="block text-xs font-bold text-gray-500 mb-1">Expiry date</label>
                                <input required type="text" placeholder="MM/YY" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#584cf4]" />
                            </div>
                            <div className="flex-1">
                                <label className="block text-xs font-bold text-gray-500 mb-1">CVC/CVV</label>
                                <input required type="password" placeholder="CVC" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#584cf4]" />
                            </div>
                        </div>
